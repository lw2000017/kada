...
def getTimeStamp():
    ...
    nowTime = int(round(t * 1000))	# round()返回浮点数x的四舍五入的值
...
def cookie_change_dict(_hhdd_):
    ...
    hhdd_dict = _hhdd_.strip('""').strip('{}')	# strip()用于移除字符串头尾指定的字符或字符序列

    hhdd = hhdd_dict.split(",")	# split()制定分隔符对字符串进行切片。这里的含义是如果有“，”就进行切割
